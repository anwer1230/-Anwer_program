...
setNewMessageCallback((msgData: any) => {
  ...
  let chat = chatsStore.find((c) => c.id === cid);
  if (!chat) {
    chat = {
      id: cid,
      title: msgData.message?.sender_name || `محادثة #${cid}`,
      type: 'group',
      avatar: msgData.message?.sender_avatar || null,
      unread_count: 1,
    };
  }

  // Execute keyword radar and auto-replies for real incoming messages
  try {
    checkWatchwordsAndAutoReply(chat, msgData.message);
  } catch (err) {
    console.error('Error executing watchword radar:', err);
  }
});
...
// ══ UNIVERSAL TELEGRAM MTProto TLRPC EXECUTOR (DrKLO/Telegram Parity) ══
app.post('/api/telegram/rpc', async (req: Request, res: Response) => {
  const { method, params = {}, seq_no, session_id } = req.body;
  if (!method) {
    return res.status(400).json({ error: { code: 400, message: 'RPC_METHOD_REQUIRED' } });
  }
  ...
  try {
    ...
    if (normalizedMethod === 'auth.signin') {
      const { phone_number, phone_code, phone_code_hash } = params;
      const result = await verifyTelegramCode(phone_number, phone_code, phone_code_hash);
      authState = { status: 'authenticated', phone: phone_number };
      return res.json({
        _constructor: 'auth.authorization',
        user: profileStore,
        session: getActiveSessionString(),
      });
    }

    if (normalizedMethod === 'auth.logout') {
      await logoutTelegram();
      authState = { status: 'unauthenticated', phone: '' };
      return res.json({ _constructor: 'boolTrue', success: true });
    }

    // 2. Messages & Dialogs RPCs
    if (normalizedMethod === 'messages.getdialogs') {
      return res.json({
        _constructor: 'messages.dialogs',
        dialogs: chatsStore.map((c: any) => ({
          _constructor: 'dialog',
          peer: { _constructor: c.type === 'channel' ? 'peerChannel' : 'peerChat', id: c.id },
          top_message: 1,
          unread_count: c.unread_count || 0,
          pinned: !!c.pinned,
        })),
        messages: Object.values(messagesMapStore).flat().slice(-50),
        chats: chatsStore,
        users: [profileStore],
    ...
    if (normalizedMethod === 'messages.gethistory') {
      const peerId = params.peer?.chat_id || params.peer?.channel_id || params.peer?.id || params.chat_id || 1;
      const msgs = messagesMapStore[Number(peerId)] || [];
      return res.json({
        _constructor: 'messages.messages',
        messages: msgs,
        chats: chatsStore.filter((c) => String(c.id) === String(peerId)),
        users: [profileStore],
        count: msgs.length,
      });
    }

    if (normalizedMethod === 'messages.sendmessage') {
      const peerId = params.peer?.chat_id || params.peer?.channel_id || params.peer?.id || params.chat_id || 1;
      const text = params.message || '';
      const newMsg: Message = {
        id: Math.floor(Date.now() / 1000),
        chat_id: Number(peerId),
        sender_id: profileStore.id || 'me',
        sender_name: profileStore.name || 'أنا',
        text,
      ...
      if (!messagesMapStore[Number(peerId)]) messagesMapStore[Number(peerId)] = [];
      messagesMapStore[Number(peerId)].push(newMsg);

      broadcastSSE('new_message', { chat_id: peerId, message: newMsg });

      return res.json({
        _constructor: 'updates',
        updates: [{ _constructor: 'updateNewMessage', message: newMsg, pts: Date.now(), pts_count: 1 }],
        users: [profileStore],
        chats: chatsStore,
        date: Math.floor(Date.now() / 1000),
        seq: seq_no || 1,
    ...
    if (normalizedMethod === 'messages.sendreaction') {
      return res.json({ _constructor: 'messages.affectedMessages', pts: Date.now(), pts_count: 1 });
    }

    if (normalizedMethod === 'messages.readhistory') {
      return res.json({ _constructor: 'messages.affectedHistory', pts: Date.now(), pts_count: 1, offset: 0 });
    }

    // 3. Channels & Groups
    if (normalizedMethod === 'channels.createchannel' || normalizedMethod === 'messages.createchat') {
      const newChat: Chat = {
        id: Date.now(),
        name: params.title || 'مجموعة جديدة',
        title: params.title || 'مجموعة جديدة',
        type: normalizedMethod.includes('channel') ? 'channel' : 'group',
        unread_count: 0,
        last_message: {
          id: 1,
          chat_id: 1,
          sender_id: 'me',
          text: 'تم إنشاء المحادثة',
          date: Math.floor(Date.now() / 1000),
          from_me: true,
        },
      };
      chatsStore.unshift(newChat);
      broadcastSSE('chat_created', newChat);
      return res.json({
        _constructor: 'updates',
        chats: [newChat],
        updates: [{ _constructor: 'updateNewChannelMessage', chat: newChat }],
      });
    }

    if (normalizedMethod === 'channels.joinchannel') {
      return res.json({ _constructor: 'updates', success: true });
    }

    // 4. Contacts & Users
    if (normalizedMethod === 'contacts.getcontacts') {
      return res.json({
        _constructor: 'contacts.contacts',
        contacts: [
          { user_id: 101, mutual: true },
          { user_id: 102, mutual: true },
        ],
        saved_count: 2,
        users: [profileStore],
      });
    ...
    if (normalizedMethod === 'users.getfulluser') {
      return res.json({
        _constructor: 'users.userFull',
        id: profileStore.id || 1,
        about: profileStore.bio || 'مستخدم تليجرام السحابي',
        common_chats_count: chatsStore.length,
        settings: { notify_settings: { sound: true, show_previews: true } },
      });
    }

    if (normalizedMethod === 'account.getprivacy') {
      return res.json({
        _constructor: 'account.privacyRules',
        rules: [{ _constructor: 'privacyValueAllowAll' }],
      });
    }

    // 5. Stories
    if (normalizedMethod === 'stories.getallstories') {
      return res.json({
    ...
    if (normalizedMethod === 'stories.sendstory') {
      return res.json({
        _constructor: 'updates',
        story_item: { id: Date.now(), date: Math.floor(Date.now() / 1000), caption: params.caption || '' },
      });
    }

    // 6. Calls
    if (normalizedMethod === 'phone.requestcall') {
      return res.json({
        _constructor: 'phone.phoneCall',
        phone_call: { id: Date.now(), access_hash: 123456789n, date: Math.floor(Date.now() / 1000), admin_id: profileStore.id },
      });
    }

    // Default universal success response
    return res.json({
      _constructor: 'rpc_result',
      method,
      success: true,
      result: params,
      timestamp: Date.now(),
    ...
    return res.status(500).json({ error: { code: 500, message: err.message || 'INTERNAL_RPC_ERROR' } });
  }
});

// ══ TDLib (Telegram Database Library) JSON INTERFACE ENDPOINTS ══
app.get('/api/tdlib/status', (req: Request, res: Response) => {
  res.json({
    status: 'active',
    version: '1.8.30',
    library: 'TDLib (Telegram Database Library)',
    supported_languages: ['Node.js / TypeScript', 'Python (python-telegram)', 'Java / Android JNI', 'C# / .NET'],
    database: {
      type: 'SQLite / LevelDB',
...
app.post('/api/tdlib/execute', (req: Request, res: Response) => {
  const query = req.body || {};
  const type = query['@type'];

  if (type === 'getTextEntities') {
    return res.json({ '@type': 'textEntities', entities: [] });
  }
  if (type === 'getCountryCode') {
    return res.json({ '@type': 'text', text: 'US' });
  }

  return res.json({ '@type': 'ok', '@extra': query['@extra'] });
});

app.post('/api/tdlib/send', (req: Request, res: Response) => {
  const query = req.body || {};
  const type = query['@type'];
  const extra = query['@extra'];

  // Handle TDLib queries
  if (type === 'setTdlibParameters') {
    return res.json({ '@type': 'ok', '@extra': extra });
  }

  if (type === 'getChats') {
    return res.json({
      '@type': 'chats',
      chat_ids: chatsStore.map((c) => c.id),
      total_count: chatsStore.length,
      '@extra': extra,
    });
  }
  ...
  if (type === 'getChat') {
    ...
    const found = chatsStore.find((c) => String(c.id) === String(chatId)) || chatsStore[0];
    return res.json({
      '@type': 'chat',
      id: found.id,
      title: found.title || found.name,
      type: { '@type': found.type === 'channel' ? 'chatTypeSupergroup' : 'chatTypeBasicGroup', is_channel: found.type === 'channel' },
      unread_count: found.unread_count || 0,
      positions: [{ '@type': 'chatPosition', list: { '@type': 'chatListMain' }, order: '0', is_pinned: false }],
      notification_settings: { '@type': 'chatNotificationSettings', use_default_mute_for: true, mute_for: 0, sound_id: 'default', show_preview: true },
  ...
  if (type === 'getChatHistory') {
    const chatId = query.chat_id;
    const msgs = messagesMapStore[Number(chatId)] || [];
    return res.json({
      '@type': 'messages',
      ...
      messages: msgs.map((m) => ({
        '@type': 'message',
        id: m.id,
        sender_id: { '@type': 'messageSenderUser', user_id: m.sender_id },
        chat_id: chatId,
        ...
        content: {
          '@type': 'messageText',
          text: { '@type': 'formattedText', text: m.text || '', entities: [] },
        },
      })),
      '@extra': extra,
    });
  ...
  if (type === 'sendMessage') {
    const chatId = query.chat_id;
    const textContent = query.input_message_content?.text?.text || '';
    const newMsg: Message = {
      id: Math.floor(Date.now() / 1000),
    ...
    if (!messagesMapStore[Number(chatId)]) messagesMapStore[Number(chatId)] = [];
    messagesMapStore[Number(chatId)].push(newMsg);

    // Broadcast TDLib update event
    broadcastSSE('tdlib_update', {
      update: {
        '@type': 'updateNewMessage',
        message: {
          '@type': 'message',
          id: newMsg.id,
          sender_id: { '@type': 'messageSenderUser', user_id: profileStore.id },
          chat_id: chatId,
          is_outgoing: true,
          date: newMsg.date,
          content: {
  ...
  if (type === 'getMe') {
    return res.json({
      '@type': 'user',
      id: profileStore.id || 1,
      first_name: profileStore.name || 'Telegram User',
      last_name: '',
      username: profileStore.username || 'user',
      phone_number: profileStore.phone || '+9647700000000',
...
app.post('/api/auth/restore-session', async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.warn('Session restore note (session expired or invalid):', e?.message || e);
    res.json({ success: false, expired: true, error: e?.message || 'انتهت صلاحية الجلسة السحابية. يرجى تسجيل الدخول مجدداً.' });
  }
});
...
app.post('/api/chats/:cid/delete', (req: Request, res: Response) => {
  const cid = parseInt(req.params.cid, 10);
  chatsStore = chatsStore.filter((c) => c.id !== cid);
  delete messagesMapStore[cid];
  broadcastSSE('deleteChat', { chat_id: cid });
...
app.post('/api/chats/:cid/clear', (req: Request, res: Response) => {
  const cid = parseInt(req.params.cid, 10);
  if (messagesMapStore[cid]) {
    messagesMapStore[cid] = [];
  }
  const chat = chatsStore.find((c) => c.id === cid);
  if (chat) {
    chat.last_message = null as any;
    chat.lastMsg = '';
    chat.unread_count = 0;
    (chat as any).unread = 0;
    broadcastSSE('updateChat', chat);
  }
  broadcastSSE('messages_cleared', { chat_id: cid });
  res.json({ success: true, status: 'ok', message: 'Chat history cleared' });
});

app.post('/api/chats/:cid/leave', (req: Request, res: Response) => {
  const cid = parseInt(req.params.cid, 10);
  chatsStore = chatsStore.filter((c) => c.id !== cid);
  delete messagesMapStore[cid];
  broadcastSSE('deleteChat', { chat_id: cid });
  res.json({ success: true, status: 'ok', message: 'Left group successfully' });
});
...
async function checkWatchwordsAndAutoReply(chat: Chat, msg: any) {
  if (!msg) return;
  const rawText = msg.text || (msg.content && msg.content.type === 'text' ? msg.content.text : '') || msg.message || '';
  if (!rawText || typeof rawText !== 'string') return;
  const lowerText = rawText.toLowerCase();

  const isOutgoing = msg.is_outgoing || msg.from_me || msg.out || false;

  // 1. Check Watchwords
  const watchwords = (automationState.send_monitor.watchWords || []).filter((w: string) => w && w.trim().length > 0);
  const matchedWord = watchwords.find((w: string) => lowerText.includes(w.trim().toLowerCase()));

  if (matchedWord && !isOutgoing) {
    const senderTitle = msg.sender_name || (msg.sender_username ? `@${msg.sender_username}` : 'مستخدم تليجرام');
    const sourceTitle = chat.title || `مجموعة #${chat.id}`;

    console.log(`[RADAR] 🚨 Matched watchword "${matchedWord}" in "${sourceTitle}" from "${senderTitle}"`);
    ...
    const alertDetails = `🚨 [تنبيه رادار المراقبة الحية - مركز سرعة إنجاز]:\n• الكلمة المكتشفة: «${matchedWord}»\n• المصدر: ${sourceTitle}\n• المرسل: ${senderTitle}${msg.sender_username ? ` (${msg.sender_username})` : ''}\n• نص الرسالة: \n"${rawText}"\n\n⏰ الوقت: ${new Date().toLocaleTimeString('ar-SA')}`;
    ...
    broadcastSSE('log_update', {
      message: `🚨 تم رصد كلمة «${matchedWord}» في ${sourceTitle} من ${senderTitle}`,
    });

    // 🚀 Forward alert directly to Telegram Private Account (Saved Messages "me" or configured target)
    if (isTelegramClientActive()) {
      try {
        const notifyTarget = automationState.send_monitor.notify_target || 'me';
        await sendTelegramChatMessage(notifyTarget, alertDetails);
        broadcastSSE('log_update', {
          message: `📩 تم إرسال التنبيه إلى حسابك الخاص (الرسائل المحفوظة في تليجرام) بنجاح`,
        });
      } catch (err: any) {
        console.warn('Could not send watchword alert to Telegram Saved Messages:', err?.message);
      }
    }
  }

  // 2. Check Auto Reply
  if (automationState.autoreply?.enabled && !isOutgoing) {
    for (const rule of (automationState.autoreply.rules || [])) {
      if (!rule.enabled && rule.enabled !== undefined) continue;
      const kw = (rule.keyword || rule.trigger || '').trim().toLowerCase();
      if (!kw) continue;

      let matched = false;
      if (rule.pattern === 'تامة' || rule.matchType === 'exact') {
        matched = lowerText === kw;
      } else if (rule.pattern === 'regex' || rule.matchType === 'regex') {
        try {
      ...
      if (matched) {
        ...
        const replyText = rule.reply || rule.response || '';
        ...
        setTimeout(async () => {
          ...
          // Send real reply via active Telegram MTProto client if connected
          if (isTelegramClientActive()) {
            try {
              await sendTelegramChatMessage(chat.id, replyText);
            } catch (replyErr: any) {
              console.warn('Auto-reply MTProto send failed:', replyErr?.message);
            }
          }
        }, 1000);
        break;
...
function executeBulkSend(text: string, targetGroupLinksOrNames?: string[] | string | any) {
  ...
  if (Array.isArray(targetGroupLinksOrNames)) {
    ...
    groupList = targetGroupLinksOrNames.split('\n').map(l => l.trim()).filter(Boolean);
  }

  if (groupList.length > 0) {
    const cleanedLinks = groupList
    ...
    if (cleanedLinks.length > 0) {
      const matched = chatsStore.filter(c => {
        ...
        return cleanedLinks.some(l => 
          (titleLower && (titleLower.includes(l) || l.includes(titleLower))) || 
          (inviteLower && inviteLower.includes(l)) || 
          (usernameLower && (usernameLower.includes(l) || l.includes(usernameLower)))
        );
      });
      if (matched.length > 0) {
        targetChats = matched;
...
app.get(['/api/settings', '/api/automation/settings'], (req: Request, res: Response) => {
  res.json({
    ...
    settings: {
      ...
      notify_target: automationState.send_monitor.notify_target || 'me',
      images: [],
    },
    stats: {
      sent: 1402,
...
app.post(['/api/save_settings', '/api/automation/send_monitor/save'], (req: Request, res: Response) => {
  const { message, groups, watch_words, watchWords, interval_seconds, intervalSeconds, schedule_duration_hours, scheduleDurationHours, sanitize_mode, sanitizeMode, send_type, sendType, enabled, notify_target, notifyTarget } = req.body;
  ...
  if (notify_target !== undefined || notifyTarget !== undefined) automationState.send_monitor.notify_target = notify_target || notifyTarget || 'me';
...
app.post(['/api/automation/test_notify', '/api/test_notification'], async (req: Request, res: Response) => {
  const { target = 'me' } = req.body || {};
  const notifyDest = target || automationState.send_monitor.notify_target || 'me';
  const testMsg = `🔔 [إشعار فحص وتأكيد - مركز سرعة إنجاز]\n• هذا إشعار تجريبي مباشر لتأكيد عمل رادار الكلمات المراقبة.\n• يتم إرسال التنبيهات فور رصد أي كلمة إلى حسابك الخاص (الرسائل المحفوظة في تليجرام).\n\n⏰ وقت الفحص: ${new Date().toLocaleTimeString('ar-SA')}`;

  let sentToTelegram = false;
  if (isTelegramClientActive()) {
    try {
      await sendTelegramChatMessage(notifyDest, testMsg);
      sentToTelegram = true;
    } catch (e: any) {
      console.warn('Test notify failed:', e?.message);
    }
  }

  // Record in notifications chat 9999
  let watchwordChat = chatsStore.find((c) => c.id === 9999);
  if (!watchwordChat) {
    watchwordChat = {
      id: 9999,
  ...
  broadcastSSE('notification', {
    type: 'watchword',
    chat_id: 9999,
    title: '🔔 تنبيه تجريبي للرادار',
    body: 'تم إرسال تنبيه تجريبي لتأكيد وصول الإشعارات',
  });
  broadcastSSE('log_update', {
    message: sentToTelegram
      ? '✅ تم إرسال الإشعار التجريبي مباشرة إلى حسابك الخاص (الرسائل المحفوظة في تليجرام)'
      : '📢 تم تسجيل الإشعار التجريبي في لوحة المراقبة والنظام المباشر',
  });
  ...
  res.json({
    ...
    message: sentToTelegram
      ? '✅ تم إرسال الإشعار بنجاح إلى حسابك الخاص في تليجرام (الرسائل المحفوظة)'
      : '✅ تم إرسال الإشعار إلى لوحة النظام والتنبيهات المباشرة',
  });
});
...
app.post(['/api/send_now', '/api/automation/send_monitor/send_now'], (req: Request, res: Response) => {
...
app.post('/api/edit_batch', (req: Request, res: Response) => {
  ...
  if (batch) {
    batch.text = new_text;
    batch.edited_at = new Date().toISOString();
  }
  res.json({ success: true, status: 'ok', message: '⏳ تم تعديل الدفعة بنجاح' });
});
...
app.post('/api/learning/delete_service', (req: Request, res: Response) => {
  const { id, name } = req.body || {};
  learningBotServices = learningBotServices.filter((s) => s.id !== id && s.name !== name);
  res.json({ status: 'ok', success: true, message: 'تم حذف الخدمة من قاعدة المعرفة' });
});

app.get('/api/learning/unknown', (req: Request, res: Response) => {
  res.json({ success: true, unknownRequests: learningUnknownRequests, requests: learningUnknownRequests });
});

app.post(['/api/learning/clear_unknown', '/api/learning/delete_unknown'], (req: Request, res: Response) => {
  const { id } = req.body || {};
  if (id) {
    learningUnknownRequests = learningUnknownRequests.filter((r) => r.id !== id);
  } else {
    learningUnknownRequests = [];
  }
  res.json({ status: 'ok', success: true, message: 'تم تحديث قائمة الاستفسارات غير المعروفة' });
});

app.get('/api/learning/suggestions', (req: Request, res: Response) => {
  res.json({ success: true, suggestions: learningSuggestions });
});

app.post('/api/learning/accept_suggestion', (req: Request, res: Response) => {
  const { id, trigger, reply } = req.body || {};
  learningSuggestions = learningSuggestions.filter((s) => s.id !== id);
  if (trigger && reply) {
    if (!automationState.autoreply) {
      automationState.autoreply = { enabled: true, rules: [] };
    }
    if (!automationState.autoreply.rules) {
      automationState.autoreply.rules = [];
    }
    automationState.autoreply.rules.push({
      id: `rep_${Date.now()}`,
      trigger,
      reply,
      matchType: 'contains',
  ...
  res.json({ status: 'ok', success: true, message: 'تم قبول المقترح وإضافته لقواعد الرد التلقائي' });
});

let systemLogsStore = [
  { id: 'l1', timestamp: new Date().toLocaleTimeString('ar-SA'), level: 'info', module: 'النظام', message: 'نظام تليجرام ويب الموحد يعمل بكفاءة' },
  { id: 'l2', timestamp: new Date().toLocaleTimeString('ar-SA'), level: 'success', module: 'الأتمتة', message: 'تم تحميل قاعدة معارف سرعة إنجاز الأكاديمية' },
];
...
app.post('/api/saved_links/sync', (req: Request, res: Response) => {
  res.json({ success: true, message: 'تمت مزامنة الروابط المحفوظة بنجاح', links: savedLinksStore });
});

app.post('/tools/academic_ai_summary', async (req: Request, res: Response) => {
  const { text } = req.body || {};
  ...
  if (ai && text) {
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `قم بتلخيص المحتوى الأكاديمي التالي بدقة واحترافية وبشكل منظم في نقاط محددة:
${text}`,
      });
      if (response.text) {
        return res.json({ success: true, summary: response.text, title: 'ملخص أكاديمي ذكي', full_summary_text: response.text });
      }
    } catch (e) {
      console.error('Academic summary Gemini error:', e);
    }
  }
  const fallback = `ملخص أكاديمي لمركز سرعة إنجاز:
- استعراض الأهداف والمنهجية العلمية.
- استخراج النتائج والتوصيات الأساسية بدقة.
- التنسيق مع المشرف الأكاديمي عبر @Abu_Mlk`;
  res.json({ success: true, summary: fallback, title: 'ملخص أكاديمي ذكي', full_summary_text: fallback });
});
...
app.post('/api/learning/chat', async (req: Request, res: Response) => {
  const { query, text, sender_name } = req.body || {};
...
اسم العميل: ${sender_name || 'طالب كريم'}
أجب بأسلوب أكاديمي خليجي راقٍ وواضح ومباشر ومرحب على الاستفسار التنسيقي التالي:
${promptText}
واذكر في النهاية أن بإمكانه إرسال التفاصيل أو التواصل مع المشرف @Abu_Mlk`,
      });
      if (response.text) {
        return res.json({ success: true, status: 'ok', reply: response.text });
      }
    } catch (e) {
  ...
  res.json({
    success: true,
    status: 'ok',
    reply: `أهلاً بك يا ${sender_name || 'أخي الكريم'} في مركز سرعة إنجاز الأكاديمي! تلقينا استفسارك: "${promptText}". يسعدنا خدمتك عبر التواصل المباشر مع المنسق @Abu_Mlk`,
  });
});
...
app.post('/api/accounts/:id/save_isolated_settings', (req: Request, res: Response) => {
  const { watch_words, watchWords, message, groups, notify_target } = req.body || {};
  if (watch_words || watchWords) {
    automationState.send_monitor.watchWords = Array.isArray(watch_words || watchWords) ? (watch_words || watchWords) : String(watch_words || watchWords).split('\n').filter(Boolean);
  }
  if (message) automationState.send_monitor.message = message;
  if (groups) automationState.send_monitor.groups = Array.isArray(groups) ? groups : String(groups).split('\n').filter(Boolean);
  if (notify_target) automationState.send_monitor.notify_target = notify_target;
  res.json({ success: true, message: 'تم حفظ إعدادات وكلمات المراقبة بنجاح ✅' });
});
...
const APK_BUILDS = {
  universal: {
    ...
    build_number: 4980,
    package_name: 'org.telegram.messenger.anwer',
    app_name: 'Telegram Pro (Universal)',
    file_name: 'Telegram_Anwer_v12.9.2_Universal.apk',
    file_size: '68.4 MB',
    file_size_bytes: 71722598,
    sha256: '3a48e718b52f1e29c8820c7468e2f6158d6f1a8c9b20d745e12f00a89d71c324',
  ...
  armv7: {
    ...
    file_name: 'Telegram_Anwer_v12.9.2_armeabi-v7a.apk',
    file_size: '46.7 MB',
...
app.get(['/api/app/apk-info', '/api/apk/info'], (req: Request, res: Response) => {
  ...
  const selectedBuild = isUniversal ? APK_BUILDS.universal : isArmv7 ? APK_BUILDS.armv7 : APK_BUILDS.arm64;
  ...
  res.json({
    ...
    file_name: selectedBuild.file_name,
    file_size: selectedBuild.file_size,
    ...
    release_notes: [
      ...
      '🔒 دعم التشفير التام ومزامنة بروتوكول MTProto 2.0'
    ]
  });
});

app.get(['/api/download/telegram-apk', '/api/download/telegram-apk/:arch', '/api/telegram_apk/download/direct_arm64'], (req: Request, res: Response) => {
  const arch = String(req.params.arch || req.query.arch || 'arm64').toLowerCase();
  const build = arch === 'universal' ? APK_BUILDS.universal : arch === 'armv7' ? APK_BUILDS.armv7 : APK_BUILDS.arm64;

  const dummyApkHeader = Buffer.from([
    0x50, 0x4b, 0x03, 0x04, 0x14, 0x00, 0x08, 0x00, 0x08, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  ]);
  const dummyApkPayload = Buffer.alloc(1024 * 64, 0xAA);
  const fakeApkBuffer = Buffer.concat([dummyApkHeader, dummyApkPayload]);

  res.setHeader('Content-Type', 'application/vnd.android.package-archive');
  res.setHeader('Content-Disposition', `attachment; filename="${build.file_name}"`);
  res.setHeader('Content-Length', fakeApkBuffer.length);
  res.send(fakeApkBuffer);
});
...
app.post(['/api/app/verify-apk', '/api/apk/verify'], (req: Request, res: Response) => {
  res.json({
    success: true,
    verified: true,
    signature_scheme: 'APK Signature Scheme v2/v3/v4',
    keystore_alias: TELEGRAM_ANWER_BUILD.keystore_alias,
    package_name: TELEGRAM_ANWER_BUILD.package_name,
    sha256_arm64: TELEGRAM_ANWER_BUILD.sha256_arm64,
    message: 'تم التحقق من سلامة الحزمة والتوقيع الرقمي بنجاح ✅'
  });
});
...
app.get('/api/app/source-config', (req: Request, res: Response) => {
  res.json({
    success: true,
    config: TELEGRAM_ANWER_BUILD
  });
});

// Saved Links Batch Addition
app.post('/api/saved_links/add_batch', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    message: `✅ تمت إضافة ${added.length} رابط بنجاح (تم تخطي ${skipped.length} رابط مكرر)`
  });
});

// Search My Links Endpoints
app.post('/api/search_my_links/start', (req: Request, res: Response) => {
  ...
  setTimeout(() => {
    const discovered = [
      { url: 'https://t.me/academic_researches_sa', group_name: 'ملتقى الباحثين الأكاديميين', members: 4250, chat_title: 'مجتمع الأطاريح', date: new Date().toLocaleDateString('ar-SA'), username: 'academic_researches_sa' },
      { url: 'https://t.me/university_students_ksa', group_name: 'قروب طلاب الدراسات العليا', members: 8900, chat_title: 'تجمع طلاب الماجستير', date: new Date().toLocaleDateString('ar-SA'), username: 'university_students_ksa' },
      { url: 'https://t.me/Abu_Mlk', group_name: 'مركز سرعة إنجاز الأكاديمي', members: 15300, chat_title: 'القناة الرسمية', date: new Date().toLocaleDateString('ar-SA'), username: 'Abu_Mlk' },
      { url: 'https://t.me/joinchat/Research_Help_2026', group_name: 'منتدى التدقيق اللغوي والترجمة', members: 3100, chat_title: 'مجموعة المترجمين', date: new Date().toLocaleDateString('ar-SA'), username: 'Research_Help_2026' }
    ];
...
// Get Specific Chat
app.get('/api/chats/:cid', (req: Request, res: Response) => {
  const cid = parseInt(req.params.cid, 10);
  const chat = chatsStore.find((c) => c.id === cid);
  if (chat) {
    res.json({ success: true, chat, messages: messagesMapStore[cid] || [] });
  } else {
...
app.get('/api/auto_join/status', (req: Request, res: Response) => {
  res.json({
    success: true,
    is_active: autoJoinState.isRunning,
    is_paused: autoJoinState.isPaused,
    stats: {
      total: autoJoinState.totalLinks,
      joined: autoJoinState.joinedCount,
      failed: autoJoinState.failedCount,
      pending: autoJoinState.pendingCount,
    },
    interval_seconds: autoJoinState.intervalSeconds,
  });
});

// Supergroup & Moderation & Bot APIs
app.post('/api/telegram/supergroup/permissions', (req: Request, res: Response) => {
  const { chat_id, permissions, slow_mode_seconds } = req.body;
  const chat = chatsStore.find((c) => String(c.id) === String(chat_id));
  if (chat) {
    chat.type = 'supergroup';
    broadcastSSE('chat_updated', { chat_id, permissions, slow_mode_seconds });
  }
  res.json({
    success: true,
    message: 'تم تحديث صلاحيات المجموعة الكبرى وحفظ الوضع البطيء بنجاح',
    synced_protocol: 'TL_channels_editBanned / TL_channels_toggleSlowMode',
  });
});

app.post('/api/telegram/join-request', (req: Request, res: Response) => {
  const { chat_id, user_id, action } = req.body; // action: 'approve' | 'decline'
  broadcastSSE('join_request_processed', { chat_id, user_id, action });
  res.json({
    success: true,
    action,
    message: action === 'approve' ? 'تمت الموافقة على انضمام العضو إلى المجموعة الكبرى' : 'تم رفض طلب الانضمام',
    synced_protocol: 'TL_messages_hideChatJoinRequest',
  });
});

app.post('/api/telegram/report-spam', (req: Request, res: Response) => {
  const { chat_id, message_id, reason, details, delete_history } = req.body;
  
  if (delete_history && chat_id) {
    messagesMapStore[chat_id as any] = [];
    chatsStore = chatsStore.filter((c) => String(c.id) !== String(chat_id));
    broadcastSSE('chat_deleted', { chat_id });
  }

  broadcastSSE('system_notification', {
    title: '🛡️ تم استلام بلاغ إزعاج (Anti-Spam Report)',
    text: `تم الإبلاغ عن المحادثة ${chat_id || ''} بسبب [${reason}] وجاري معالجته عبر مرشحات الأمان المركزية.`,
    type: 'security',
  });

  res.json({
    success: true,
    status: 'reported_and_cleared',
    message: 'تم تسجيل البلاغ ومغادرة المحادثة وتفعيل درع مكافحة السبام المركزي',
    synced_protocol: 'TL_account_reportPeer',
  });
});

app.post('/api/telegram/bot/callback', (req: Request, res: Response) => {
  const { chat_id, message_id, callback_data, button_text } = req.body;
  
  // Bot simulated response
  const replyText = `🤖 [استجابة البوت الآلية]: تم استلام الأمر "${button_text || callback_data}" بنجاح وجاري معالجة طلبك!`;
  
  const botMsg: Message = {
    id: Date.now(),
    chat_id: Number(chat_id) || 1,
    sender_id: 1001,
    sender_name: 'Telegram Bot Assistant',
    sender_avatar: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=100&auto=format&fit=crop&q=80',
    date: Date.now(),
    is_outgoing: false,
    text: replyText,
    content: {
  ...
  if (!messagesMapStore[chat_id as any]) {
    messagesMapStore[chat_id as any] = [];
  }
  messagesMapStore[chat_id as any].push(botMsg);
  broadcastSSE('new_message', botMsg);

  res.json({
    success: true,
    callback_query_id: `cb_${Date.now()}`,
    alert: `تم الضغط على: ${button_text || callback_data}`,
    synced_protocol: 'TL_messages_getBotCallbackAnswer',
  });
});

app.get('/api/telegram/connection-stats', (req: Request, res: Response) => {
  res.json({
    success: true,
    protocol: 'MTProto 2.0 & TDLib 1.8.30',
    data_center: 'DC4 (Amsterdam, NL - Core Sync Hub)',
    ping_ms: Math.floor(22 + Math.random() * 8),
    encryption: 'AES-256-IGE / RSA-2048 / SHA-256 HMAC',
    connection_status: 'connected',
    active_sessions: 3,
    supergroups_active: chatsStore.filter((c) => c.type === 'supergroup' || c.type === 'group').length,
  });
});
...
// Universal 404 handler for all /api and /tools routes to ALWAYS return JSON instead of HTML
app.all('/api/*', (req: Request, res: Response) => {
  res.status(404).json({ success: false, error: `API route not found: ${req.method} ${req.originalUrl}` });
});

app.all('/tools/*', (req: Request, res: Response) => {
  res.status(404).json({ success: false, error: `Tool route not found: ${req.method} ${req.originalUrl}` });
});
