...
// ── PER-ACCOUNT ISOLATED STORAGE CACHING ──────────────────────────────────

export function getAccountCacheKey(accountId: string, suffix: string): string {
  const cleanId = (accountId || 'default').replace(/[^a-zA-Z0-9_-]/g, '_');
  return `tg_acc_${cleanId}_${suffix}`;
}

export function saveAccountCachedChats(accountId: string, chats: any[]): void {
  if (!accountId || !Array.isArray(chats)) return;
  const key = getAccountCacheKey(accountId, 'chats');
  safeSetJson(key, chats);
}

export function getAccountCachedChats(accountId: string): any[] {
  if (!accountId) return [];
  const key = getAccountCacheKey(accountId, 'chats');
  return safeGetJson<any[]>(key, []);
}

export function saveAccountCachedMessages(accountId: string, chatId: string | number, messages: any[]): void {
  if (!accountId || !chatId || !Array.isArray(messages)) return;
  const cidStr = String(chatId).replace('-100', '').replace('-', '');
  const key = getAccountCacheKey(accountId, `msgs_${cidStr}`);
  safeSetJson(key, messages.slice(-MAX_MESSAGES_PER_CHAT));

  const listKey = getAccountCacheKey(accountId, 'chat_ids');
  const list = safeGetJson<string[]>(listKey, []);
  if (!list.includes(cidStr)) {
    list.push(cidStr);
    safeSetJson(listKey, list);
  }
}

export function getAccountCachedMessages(accountId: string, chatId: string | number): any[] {
...
export function getAllAccountCachedMessages(accountId: string): Record<string, any[]> {
  if (!accountId) return {};
  const result: Record<string, any[]> = {};
  const listKey = getAccountCacheKey(accountId, 'chat_ids');
  const chatIds = safeGetJson<string[]>(listKey, []);
  for (const cid of chatIds) {
    const msgs = getAccountCachedMessages(accountId, cid);
    if (msgs && msgs.length > 0) {
      result[cid] = msgs;
    }
  }
...
export function saveAccountCachedUserProfile(accountId: string, user: any): void {
  if (!accountId || !user) return;
  const key = getAccountCacheKey(accountId, 'user_profile');
  safeSetJson(key, user);
}

export function getAccountCachedUserProfile(accountId: string): any | null {
  if (!accountId) return null;
  const key = getAccountCacheKey(accountId, 'user_profile');
  return safeGetJson<any | null>(key, null);
}

export function saveAccountCachedSettings(accountId: string, settings: any): void {
  if (!accountId || !settings) return;
  const key = getAccountCacheKey(accountId, 'settings');
  safeSetJson(key, settings);
}

export function getAccountCachedSettings(accountId: string): any | null {
  if (!accountId) return null;
  const key = getAccountCacheKey(accountId, 'settings');
  return safeGetJson<any | null>(key, null);
}

export function clearAccountStorageCache(accountId: string): void {
  if (!accountId || typeof window === 'undefined' || !window.localStorage) return;
  try {
    const listKey = getAccountCacheKey(accountId, 'chat_ids');
    const chatIds = safeGetJson<string[]>(listKey, []);
    chatIds.forEach((cid) => {
      localStorage.removeItem(getAccountCacheKey(accountId, `msgs_${cid}`));
    });
    localStorage.removeItem(getAccountCacheKey(accountId, 'chats'));
    localStorage.removeItem(getAccountCacheKey(accountId, 'chat_ids'));
    localStorage.removeItem(getAccountCacheKey(accountId, 'user_profile'));
    localStorage.removeItem(getAccountCacheKey(accountId, 'settings'));
    localStorage.removeItem(getAccountCacheKey(accountId, 'session'));
    console.log(`[StorageCache] Isolated cache cleared for account: ${accountId}`);
  } catch (e) {
    console.error('[StorageCache] Error clearing account cache:', e);
  }
}
