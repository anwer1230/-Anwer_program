      ...
      {activeContextMenuMsg && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => {
                const messageLink = `https://t.me/c/${activeContextMenuMsg.chat_id}/${activeContextMenuMsg.id}`;
                navigator.clipboard?.writeText(messageLink);
                showToast('🔗 تم نسخ رابط الرسالة المباشر للحافظة');
                setActiveContextMenuMsg(null);
              }}
              className="w-full text-right p-2.5 hover:bg-slate-800 rounded-xl flex items-center gap-2.5 text-slate-200 hover:text-cyan-400 transition-colors"
            >
            ...
            <button
              onClick={() => {
                onReportSpam?.(activeContextMenuMsg);
                setActiveContextMenuMsg(null);
              }}
              className="w-full text-right p-2.5 hover:bg-slate-800 rounded-xl flex items-center gap-2.5 text-slate-200 hover:text-rose-400 transition-colors"
            >
              <ShieldAlert className="w-4 h-4 text-rose-400" />
              <span>إبلاغ عن محتوى أو إزعاج (Report)</span>
            </button>
