...
const STORAGE_KEY = 'telegram_autojoin_settings';
...
export const AutoJoinTab: React.FC<AutoJoinTabProps> = ({
  ...
  const [linksText, setLinksText] = useState(initial.linksText);
  const [delay, setDelay] = useState(initial.delay);
  const [maxRetries, setMaxRetries] = useState(initial.maxRetries);
  const [fetchExternal, setFetchExternal] = useState(initial.fetchExternal);
  const [searchByName, setSearchByName] = useState(initial.searchByName);
  ...
  }, [linksText, delay, maxRetries, fetchExternal, searchByName]);
