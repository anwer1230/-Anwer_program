...
export const SendMonitorTab: React.FC<SendMonitorTabProps> = ({
  ...
  const [notifyTarget, setNotifyTarget] = useState('me');
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      addLog(result.success ? '✅ تم حفظ الإعدادات ووجهة الإشعارات بنجاح' : '❌ فشل الحفظ: ' + (result.message || 'خطأ'));
    } catch (err: any) {
  ...
  const handleTestNotify = async () => {
    ...
    addLog('🔔 جاري إرسال إشعار تجريبي إلى حسابك الخاص...');
    try {
      ...
      const result = await res.json();
      if (result.success) {
        addLog(result.message || '✅ تم إرسال الإشعار بنجاح');
        playBeep();
      } else {
        addLog('❌ فشل الإرسال التجريبي: ' + (result.message || 'خطأ'));
      }
    } catch (e: any) {
      addLog('❌ تعذر إرسال الإشعار: ' + e.message);
    } finally {
      ...
      <div className="w-full max-w-[480px] bg-white min-h-[90vh] p-4 rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.06)] border border-[#e9edf2] flex flex-col gap-2.5">
        ...
        <div className="bg-[#f8f9fc] rounded-xl p-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-[#e9edf2]">
          ...
          <textarea
            ...
            placeholder="ضع كل مجموعة في سطر..."
        ...
        {/* كلمات المراقبة والتنبيه إلى الحساب الخاص */}
        <div className="bg-[#f8f9fc] rounded-xl p-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-[#e9edf2] space-y-2.5">
          <div className="flex items-center justify-between">
            <span className="text-[#5a6b7c] text-[13px] font-medium flex items-center gap-1.5">
              <span>🔑 كلمات المراقبة والرصد</span>
              <span className="text-[11px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">
                {watchWords.split('\n').filter((w) => w.trim()).length} كلمات
              </span>
            </span>
            <button
              type="button"
              ...
              className="text-[11px] text-[#2a92e7] hover:text-blue-700 font-semibold flex items-center gap-1 bg-blue-50 hover:bg-blue-100 px-2.5 py-1 rounded-lg transition-colors border border-blue-200"
            >
              <i className={`fas ${isTestingNotify ? 'fa-spinner fa-spin' : 'fa-bell'}`} />
              <span>{isTestingNotify ? 'جاري الإرسال...' : '🔔 فحص إشعار لحسابي'}</span>
            </button>
          </div>
          <textarea
            id="watchWords"
            ...
            placeholder="اكتب كلمة في كل سطر (مثال: بحث، مشروع، تكليف، حل، واجب)..."
          ...
          {/* وجهة الإشعارات */}
          <div className="bg-white p-2.5 rounded-lg border border-[#d9e0e8] space-y-1.5">
            <div className="text-[12px] text-[#5a6b7c] font-medium flex items-center gap-1">
              <span>🎯 إرسال التنبيهات المكتشفة فوراً إلى:</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <label className={`flex items-center gap-1.5 p-1.5 rounded-md border cursor-pointer transition-all ${notifyTarget === 'me' ? 'border-[#2a92e7] bg-blue-50 font-semibold text-blue-900' : 'border-gray-200 text-gray-700'}`}>
                <input
                  type="radio"
                  name="notifyTargetChoice"
                  value="me"
                  checked={notifyTarget === 'me'}
                  onChange={() => setNotifyTarget('me')}
                  className="accent-[#2a92e7]"
                />
              ...
              <label className={`flex items-center gap-1.5 p-1.5 rounded-md border cursor-pointer transition-all ${notifyTarget !== 'me' ? 'border-[#2a92e7] bg-blue-50 font-semibold text-blue-900' : 'border-gray-200 text-gray-700'}`}>
                <input
                  ...
                  value="custom"
                  checked={notifyTarget !== 'me'}
                  onChange={() => setNotifyTarget('@Abu_Mlk')}
                  className="accent-[#2a92e7]"
                />
                <span>معرف تليجرام مخصص</span>
              </label>
            </div>
            {notifyTarget !== 'me' && (
              <input
                ...
                placeholder="أدخل المعرف مثل @username أو ID..."
                className="w-full mt-1 p-2 rounded border border-[#d9e0e8] text-xs outline-none focus:border-[#2a92e7]"
              />
            )}
            <div className="text-[11px] text-[#6c7a8a] flex items-center gap-1">
              <i className="fas fa-info-circle text-blue-500" />
              <span>عند رصد أي كلمة في المجموعات، سيصلك إشعار فوري وتنبيه في الرسائل المحفوظة بحسابك.</span>
            </div>
          </div>
        </div>
