...
const STORAGE_KEY = 'telegram_rotating_settings';
...
export const RotatingTab: React.FC<RotatingTabProps> = ({
  ...
  const localInitial = loadSaved();

  const [messages, setMessages] = useState<string[]>(() => {
    if (status.messages?.length) return status.messages;
    if (localInitial?.messages?.length) return localInitial.messages;
    return ['', '', '', '', ''];
  });
  const [groups, setGroups] = useState<string>(() => {
    if (status.groups?.length) return status.groups.join('\n');
    if (localInitial?.groups) return localInitial.groups;
    return '';
  });
  const [intervalVal, setIntervalVal] = useState<number>(() => {
    if (status.interval) return status.interval;
    if (localInitial?.intervalVal) return localInitial.intervalVal;
    return 5;
  });
